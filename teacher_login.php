...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($username) || empty($password)) {
        ...
        if ($result && mysqli_num_rows($result) === 1) {
            ...
            // Verify password (using the default hash for demo)
            if (password_verify($password, $user['password'])) {
                ...
                redirect('teacher_dashboard.php', 'Welcome back, ' . $user['full_name']);
            } else {
...
<head>
    ...
    <title>Teacher Login - Aparna Public School</title>
